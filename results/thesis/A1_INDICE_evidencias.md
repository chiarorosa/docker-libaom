...
| Decomposição do contexto causal | de A para A+B, 1,60× a 25% com as 3 sementes de A+B superando as 3 de A; de A para A+C, sem separação e com o dobro da dispersão; A+B domina A+B+C em toda a fronteira | mesma vara, receita única | `RESULTADOS_rpp_escada_informacional.md §4.3 a §4.5` | `results/models/oracle_regret_rpp/frontier.csv`; `rpp_ladder/training.csv` | `rpp_ladder.py` | sim |
