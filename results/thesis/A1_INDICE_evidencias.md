...
| Hierarquia no crivo (reg_frac @ cost_red 25%, menor é melhor), receita fixa + grade densa | variância 0,0374; ConvNeXt-CE 0,0272; ConvNeXt α=3 0,0219; A (24 col) 0,0053; A+B (32 col) 0,0033; A+C (28 col) 0,0064; A+B+C (36 col) 0,0040 | 6 seqs held-out (val+teste), **3.808.703 nós**; degraus com 3 sementes | `RESULTADOS_rpp_escada_informacional.md §3` | `results/models/oracle_regret_rpp/{ranking,frontier}.csv` | `oracle_regret.py`; `rpp_ladder.py` | sim |
| Decomposição do contexto causal | de A para A+B, 1,60× a 25% com as 3 sementes de A+B superando as 3 de A; de A para A+C, sem separação e com o dobro da dispersão; A+B domina A+B+C de 14,2% de redução de custo em diante, invertendo-se abaixo disso | mesma vara, receita única | `RESULTADOS_rpp_escada_informacional.md §4.3 a §4.5` | `results/models/oracle_regret_rpp/frontier.csv`; `rpp_ladder/training.csv` | `rpp_ladder.py` | sim |
| Confundidor de treino na hierarquia anterior | mesmas 24 colunas: braço destilado 0,0121 contra braço de receita fixa 0,0053 a 25% — 2,25× atribuível ao procedimento, não aos atributos | mesma vara | `RESULTADOS_rpp_escada_informacional.md §4.2` | `results/models/oracle_regret_rpp/frontier.csv` | `rpp_ladder.py` | sim |
| Grade de limiares da variância — interpolação sanada | medido 0,0374 a 25% (τ=0,980 → 25,20% de redução) contra 0,0573 interpolado: superestimava em 1,53× | grade própria de 26 τ | `RESULTADOS_rpp_escada_informacional.md §4.1` | `results/models/oracle_regret_rpp/frontier.csv` | `oracle_regret.py`, `VARIANCE_TAUS` | sim |
| ConvNeXt com alvo de perda de otimalidade — **retratado**: confirmado fracamente, não refutado | sob corpus, escalonamento e seleção idênticos (`--alpha 0` degenera o peso em CE pura), α=3 é **melhor** que α=0 em todos os pontos de 10% em diante: 0,0219 contra 0,0272 a 25%. O registro anterior de piora de 1,06×–3,80× era artefato de confundimento de corpus | mesma vara held-out | `RESULTADOS_auditoria_convnext_corpus.md §3`; `RESULTADOS_rpp_escada_informacional.md §4.8` | `results/models/oracle_regret_rpp/ranking.csv` | `train_surrogate_regret.py`; `oracle_regret.py` | sim |
| Corpus do ConvNeXt-CE legado — contaminação declarada | `surrogate_real` treinado em `results/dataset` (4 seqs, cq32 único, 2 quadros) com `train_seqs=None`, `val_seqs=['Jockey']`: HoneyBee entrou no treino e Jockey selecionou o checkpoint, ambas na vara held-out | leitura da chave `args` do checkpoint | `RESULTADOS_auditoria_convnext_corpus.md §1 e §2` | `results/models/surrogate_real/surrogate_best.pt` | `data.py:77-93` | sim |
| Capacidade não é o gargalo do ConvNeXt | `val_wloss` 0,903984 (fusão 256) contra 0,895092 (128) sob α=0 e corpus correto — piora de 1,0%. O registro anterior, de 0,16%, comparava dois braços de α=3 | mesmo treino, dois checkpoints | `RESULTADOS_auditoria_convnext_corpus.md §3` | `results/models/surrogate_ce_h9{,_f256}/` | `train_surrogate_regret.py` | sim |
...
| Fechamento da família de pixels | **cinco vias não implantadas** — ConvNeXt-CE, ConvNeXt-perda de otimalidade, GNN/Approach B, bloco D, bloco D' — mas **quatro hipóteses refutadas**: a do alvo de perda de otimalidade foi confirmada fracamente, e aquela via não foi implantada por permanecer 4,1× atrás da representação compacta | consolidação | `RESULTADOS_auditoria_dominio_pixels.md §6.2`; `RESULTADOS_rpp_escada_informacional.md §4.8 e §4.9`; `INVENTARIO_solucoes.md §7` | múltiplos, listados acima | múltiplos | sim |
