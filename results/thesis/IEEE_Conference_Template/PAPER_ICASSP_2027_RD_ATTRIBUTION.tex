...
and a fixed budget of 30 epochs. All four share this recipe and differ only in
which columns they read, which is what makes the verdict fall on information
rather than on capacity or on training procedure. Each is trained with three seeds
...
The first is exposure. The arm was trained on the same corpus and under the same
split as the tabular representations, so none sees data that another does not. The
...
misclassification would incur, were trained under identical corpus, schedule, and
selection criterion. The cost-sensitive objective is better at every operating
