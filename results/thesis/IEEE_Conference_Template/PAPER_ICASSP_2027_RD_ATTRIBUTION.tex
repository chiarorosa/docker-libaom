...
AV1 encodes each frame as a grid of super blocks of 64$\times$64 samples, each
recursively divided by a search over a quad-tree of square nodes of 64, 32, 16,
and 8 samples. At every node the encoder may evaluate ten partition shapes,
...
resolution (3840$\times$2160), 8 bits and 4:2:0 chroma subsampling. Each sequence
...
information that is missing rather than to the capacity available: the deep arm
reads the same pixels as rung A and the gap is not closed by capacity, whereas
eight columns of encoder state close a further factor of 1.60 with 4291
parameters per level. It is important to emphasize that the deep arm is
