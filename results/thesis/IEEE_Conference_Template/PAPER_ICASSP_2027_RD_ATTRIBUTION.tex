...
fixed budget of 30 epochs. Every rung shares this one recipe and differs only in
which columns it reads, which is what makes the verdict fall on information rather
than on capacity or on training procedure. Each rung is trained with three seeds
...
the corpus is 4K only, so whether the causal neighborhood carries the same weight
