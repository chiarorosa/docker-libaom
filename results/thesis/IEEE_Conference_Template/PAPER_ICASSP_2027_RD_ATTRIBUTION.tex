...
\begin{figure}[!t]
\centering
% Grayscale variant. The color one is figura2_fronteira_perda.pdf, same geometry;
% both from src/scripts/benchmark/plot_loss_frontier_icassp_fig2.py, which reads
% frontier.csv and refuses to run if it disagrees with Table I.
...
operating point. At 25\%, isolated variance loses 37.4
...
effect is no longer ambiguous, as A+B reaches 3.3 units against 4.0 for A+B+C and
all nine pairwise seed comparisons agree. Fig.~\ref{fig_frontier} places the
crossover at 14.2\% of cost reduction: below it the two are within 0.05 units of
...
against 27.2 units under the same objective, and below 12\% of cost reduction the
