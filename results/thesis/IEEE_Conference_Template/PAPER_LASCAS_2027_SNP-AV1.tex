...
the inset of Fig.~\ref{fig_space} shows the two prices as the two slopes that
leave the same operating point. Its stronger calibration costs 0.040 over the
...
\begin{figure}[!t]
  ...
  \caption{Operating space of SNP-AV1 over the CTC Class A1 4K sequences. Both
  axes are measured against the libaom v3.10.0 anchor at \texttt{cpu-used=0},
  which is their origin and lies outside the plotted range. Inside each
  calibration the three points differ only in the strength of the second pruner,
  and the dashed line is the threshold knob of the first stage. The inset
  magnifies the balanced calibration, where the two prices, in BD-BR points per
  point of time savings, leave the same operating point.}
  \label{fig_space}
\end{figure}
...
residue; the tight cluster of the three aggressive points in
Fig.~\ref{fig_space} is that collapse.
