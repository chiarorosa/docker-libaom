...
\cite{han2021,chen2018}. This coding efficiency comes at the price of
computational effort: AV1 was shown to be superior to HEVC under certain
conditions \cite{grois2016}, but encoding took twice as long \cite{layek2017}, a
problem aggravated during UHD 4K content processing.
...
64$\times$64 pixels, and the structure that subdivides it is called partition
...
shapes; and the two 4-way shapes, of 4:1 aspect ratio. The AB and the 4-way
...
\begin{figure}[!t]
  ...
  families used in this work, with the share of local search time
...
the first evaluation is blind to any rate-distortion number; one acting after
NONE has the rate, distortion and RD cost of the undivided candidate, at the
price of never saving the cost of producing them.
...
extended shape exceeds 7.22\%, which is why the weight of the family went
unnoticed, and the cost concentrates in the large nodes, reaching 50.0\% at
32$\times$32 and 51.0\% at 64$\times$64 pixels, the nodes a 4K frame produces in
abundance.
...
\begin{figure}[!t]
  ...
  cascade of conditions each applies.}
  \label{fig_snp}
\end{figure}
...
the node and the whole subtree below it; it pays for that reach by deciding blind
to rate-distortion. The second acts only on the nodes that survive the first and
reads the measured RD cost of NONE, over a narrower scope: the AB and 4-way
...
their granularity and anisotropy; the last four carry the dequantization step of
the luma DC coefficient, the normalized position and the node depth.
...
with it, so the models trained on
...
guard is necessary because a binary that writes at every node cannot measure
acceleration.
...
receiver operating characteristic curve of the second from 0.890 to 0.902.
...
decided as NONE and the whole subtree is cut, the primary lever of time
...
savings below twice that value is not resolved, while the BD-BR values are exact.
The anchor is the exhaustive partition search itself, and no native speed preset
is placed beside SNP-AV1: a preset reconfigures every coding stage, so a
side-by-side reading would rank arrangements of different scope rather than
partition-search techniques.

Fig.~\ref{fig_space} places the six measured operating points against that
anchor. The balanced calibration spans time savings of 17.72\% to 19.81\% at
...
the time savings average 18.74\% with a standard deviation of 3.40 percentage
points, from 13.78\% on NocturneDance to 22.89\% on Tango, and the BD-BR averages
0.586\% with a standard deviation of 0.278, from 0.168\% to 1.139\% on those same
two sequences. The aggressive calibration keeps that shape and lifts the level,
to 31.68\% $\pm$ 4.17 of time savings, from 26.03\% to 38.80\%, at 1.409\%
$\pm$ 0.377 of BD-BR, from 0.934\% to 2.206\%. This dispersion is content and not
measurement, since it is more than ten times the standard deviation of the
repeated encodings.
...
calibration and of 22 at the aggressive one, the rising price the
aggregate showed. The two extremes are the two ends of one mechanism: the
time a sequence yields is bounded by how often the exhaustive search settles for
the undivided candidate, since committing NONE is what removes a subtree, and the
efficiency it cedes is set by how often the first stage commits where that search
would not have. The coupling between the two columns is visible at the balanced
point, where they correlate by rank at 0.79 ($p = 0.02$), and dissolves at the
aggressive one, at 0.31 ($p = 0.46$), which may be attributed to a threshold
loose enough to commit nodes almost everywhere.
...
sequences of the CTC Class A1 under the AI configuration. Measured marginally
over the first, the second pruner adds 1.02 percentage points of time savings for
0.018 of BD-BR, about 3.4 times cheaper than buying the same time from the
thresholds of the first stage, and it is the better of the two in six of the
eight sequences; the advantage collapses on the aggressive base, where the first
stage commits the nodes first. Besides, to the best of the authors' knowledge,
this is the first
