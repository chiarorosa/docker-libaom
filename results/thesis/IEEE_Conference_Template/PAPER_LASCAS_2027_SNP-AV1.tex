...
\begin{figure}[!t]
  ...
  \caption{Operating space of SNP-AV1 over the CTC Class A1 4K sequences, against
  the libaom v3.10.0 anchor at \texttt{cpu-used=0}: (a) balanced calibration and
  (b) aggressive calibration, each on its own axes. In both panels the three
  points differ only in the strength of the second pruner, the dashed line is the
  threshold knob of the first stage, which leaves the base of the family in (a)
  and reaches it in (b), and the shaded band is the 0.46-point measurement
  resolution taken from that base. The two prices annotated in (a) are in BD-BR
  points per point of time savings.}
  \label{fig_space}
\end{figure}
...
residue. Fig.~\ref{fig_space}(b) is that collapse: the deployed point stays
inside the measurement resolution of its own base, which in Fig.~\ref{fig_space}(a)
it leaves well behind.
