...
the left window of Fig.~\ref{fig_space} shows the two prices as the two slopes
that leave the same operating point. Its stronger calibration costs 0.040 over the
...
\begin{figure}[!t]
  ...
  \caption{Operating space of SNP-AV1 over the CTC Class A1 4K sequences,
  measured against the libaom v3.10.0 anchor at \texttt{cpu-used=0}, which is the
  origin of both axes. The time axis is broken into two windows of the same
  scale, the balanced calibration on the left and the aggressive one on the
  right, over a common BD-BR axis. Inside each window the three points differ
  only in the strength of the second pruner; the dashed line is the threshold
  knob of the first stage, and the two annotated slopes are the prices of the two
  levers, in BD-BR points per point of time savings.}
  \label{fig_space}
\end{figure}
...
residue. The right window of Fig.~\ref{fig_space} is that collapse: at the same
scale as the left one, the whole aggressive family spans 0.65 percentage points
of time savings, against the 2.09 the second pruner alone covers over the
balanced base.
