...
| `compare_students.py` | Compara dois bundles H9a por recall/macro-F1 por nível (ablação B4). Ver `RESULTADOS_modelagem_B4_ponderacao_classe.md` | 2 bundles + dataset | (console) |
| `train_student_h9.py` (flag `--class-weight`) | B4: treino com ponderação de classe por nível → `student_h9a_cw`. **Resultado negativo** (não implantar); valida o treino sem peso | dataset | `student_h9a_cw/students.pt` |
| `train_student_h9.py` (flag `--feature-set h9a_b1`) | B1: +6 atributos RD hereditários (pai + irmãos anteriores, `features.py:node_features_h9a_b1`) → `student_h9a_b1` (42 feat). **Resultado negativo** (degrada o crivo); valida o vetor de 36 | dataset | `student_h9a_b1/students.pt` |
| `b2_tau_per_qindex.py` | B2: τ por qindex (sem re-treino), τ global vs estratificado-ótimo no crivo. **Positivo relativo (~2× menos regret) mas absoluto imaterial** (< ruído do encoder); grátis (C lê τ do ambiente). Ver `RESULTADOS_modelagem_B2_tau_qindex.md` | student_h9a + dataset | `b2_tau_qindex/*.csv` |
| `b3_horz_vert.py` | B3: colapso 10→4 (NONE/SPLIT/HORZ/VERT), acurácia direcional condicional. **Sinal confirmado** (69% vs 52% baseline, +16,7pp) mas rect-off é ação secundária + exige política em C + encodes. Ver `RESULTADOS_modelagem_B3_horz_vert.md` | dataset | `student_h9a_4cls/` |
